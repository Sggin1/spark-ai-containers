...
- **aarch64 + SM121 optimization gap.** Steady-state generation is ~7-8 tok/s via direct transformers, compared to ~50 tok/s for the same model family via vLLM (NVFP4 quantized, Marlin backend). The mamba-ssm CUDA kernels (`selective_scan_cuda`, `causal_conv1d_cuda`) were developed and optimized for x86_64 + SM80/SM90 (A100/H100). On DGX Spark's aarch64 + SM121, they compile and produce correct results but are not tuned for this architecture. This is part of a broader gap: mamba-ssm, NVFP4 native tensor core support, and CUTLASS kernels all need SM12x-specific optimization for Spark/GB10 to reach its potential. The community and NVIDIA improved NVFP4/SM12x support substantially through mid-2026 (see [nvfp4](../nvfp4/)), but mamba-ssm kernel optimization for aarch64 + SM121 remains an open area.

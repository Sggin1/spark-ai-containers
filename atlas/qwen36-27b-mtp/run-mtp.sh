#!/usr/bin/env bash
# Maintainer-requested test: Qwen3.6-27B-FP8 throughput, Atlas with/without MTP, vs vLLM.
#
# Per maintainer's Discord reply (2026-05-07):
#   "It's Qwen/Qwen3.6-27B-FP8 the same parameters should work that you have
#    for 35-A3B, with/without speculative!"
#
# Reuses bench.py from the morning's run-bench-2026-05-07/phase2/.
#
...
python3 "$BENCH_PY" \
  ...
  2>&1 | tee "$DIR/logs/atlas-no-mtp-bench.log" | grep -E "^=== |tok/s"
...
python3 "$BENCH_PY" \
  ...
  2>&1 | tee "$DIR/logs/atlas-mtp-bench.log" | grep -E "^=== |tok/s"
...
python3 "$BENCH_PY" \
  ...
  2>&1 | tee "$DIR/logs/vllm-bench.log" | grep -E "^=== |tok/s"

...
def run(url: str, model: str, runs_per_prompt: int, concurrency: int) -> dict:
    ...
    if concurrency == 1:
        for name, prompt, max_tok, i in jobs:
            ...
            print(f"  [{name}#{i}] {r['completion_tokens']}tok in {r['elapsed_s']}s = {r['tok_per_s']} tok/s (ttft={r.get('ttft_ms')}ms)", flush=True)
    else:
        with ThreadPoolExecutor(max_workers=concurrency) as ex:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    r["iteration"] = i
                    out["results"].append(r)
                    print(f"  [{name}#{i} c={concurrency}] {r['completion_tokens']}tok / {r['elapsed_s']}s = {r['tok_per_s']} tok/s", flush=True)

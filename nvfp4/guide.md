...
> **Snapshot: 2026-03-26** (friction-era how-we-got-there). **Not the default install path.**  
> **Current narrative + recipes:** [`README.md`](README.md) · [`../recipes/`](../recipes/).  
> **June 2026 corrections (still apply):** stock **vLLM ≥ v0.19** builds working sm_121 NVFP4 (no eugr required); native SM120/121 CUTLASS NVFP4 GEMM (vLLM PR #40082, 2026-05-20); `VLLM_NVFP4_GEMM_BACKEND` / `VLLM_USE_FLASHINFER_MOE_FP4` **deprecated** → `--linear-backend` / `--moe-backend` (per-model); llama.cpp NVFP4 GPU-accelerated. Memory ideas (KV util, `--enforce-eager`, floor ~0.2) still hold; absolute GB/tok/s are March numbers. Host baseline noted June 2026: **Driver 580.159.03 · CUDA 13.0.2**.
...
## Files (after 2026-08-06 combine)

| File | Description |
|------|-------------|
| [`README.md`](README.md) | Current NVFP4 narrative (friction → working) |
| `guide.md` | This document (March 2026 120 GB → 32 GB write-up) |
| [`memory.md`](memory.md) | Creep / KV / spike taxonomy |
| [`archive/`](archive/) | Date-stamped landscape + lab notebooks |
